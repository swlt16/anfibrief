...
$(PDFDIR)/brief_%.pdf: $(LETTERDIR)/brief_%.tex plaene $(MEDIADIR)/fsilogo.pdf $(addprefix $(SRCDIR)/,termine.sty termine.tex termine-daten.tex misc.tex einleitung.tex config.tex mailinglisten.tex tuebingen.tex ps_zulassung.tex env.tex)
	if [ ! -d $(OUTDIR) ]; then mkdir $(OUTDIR); fi
	latexmk -output-directory=$(OUTDIR) -pdf -pdflatex="pdflatex" $<
	if [ ! -d $(PDFDIR) ]; then mkdir $(PDFDIR); fi
	mv $(OUTDIR)/$(notdir $(<:.tex=.pdf)) $(PDFDIR)/$(notdir $(<:.tex=.pdf))
